import time
import board
import busio
import digitalio
import time

# LCD Source code is at https://github.com/dhalbert/CircuitPython_LCD
from lcd.lcd import LCD
from lcd.i2c_pcf8574_interface import I2CPCF8574Interface


score_button=digitalio.DigitalInOut(board.GP2) #Initalize score_button and attach it to GP3
score_button.direction = digitalio.Direction.INPUT
score_button.pull = digitalio.Pull.UP
score_button2=digitalio.DigitalInOut(board.GP3) #Initalize score_button and attach it to GP3
score_button2.direction = digitalio.Direction.INPUT
score_button2.pull = digitalio.Pull.UP

#ball_count_button=digitalio.DigitalInOut(board.GP3) #Initalize ball_count_button and attach it to GP3
#ball_count_button.direction = digitalio.Direction.INPUT
#ball_count_button.pull = digitalio.Pull.UP

score_button.value
score_button2.value
# Initialize the score variable
score = 0
# Initialize the ball count variable to 3 because the gamestarts with three balls
ball_count= 3

# Initialize I2C bus.
# The Raspberry Pi pico has a number of pin pairs that can be used for I2C.
# One pin is SCL (clock) and the other is SDA (data).  See
# a pin diagram at
i2c = busio.I2C(board.GP1, board.GP0)

# Talk to the LCD at I2C address 0x27.
lcd = LCD(I2CPCF8574Interface(i2c, 0x27), num_rows=2, num_cols=20)
lcd.set_backlight(True)

lcd.clear()
    # Start at the first line, fifth column (numbering from zero).
lcd.set_cursor_pos(0, 1)
lcd.print("Plant VS Zombie")
time.sleep(2)
lcd.clear()

while True:
        # Bump the score for the next iteration of the loop
    #Increases Score value by 5 when pressed
    if score_button.value == False:
        score= score+5
        print("increasing_score")
      #  score_button.value = True

        # Bump the score for the next iteration of the loop
    #Increases Score value by 5 when pressed
    if score_button2.value == False:
        score= score+5
        print("increasing_score")
      #  score_button.value = True

    #Increases ball count value by 1 when pressed
 #   if ball_count_button.value == False:
 #       print("decreasing_ballcount")
  #      ball_count=ball_count-1
  #   #   ball_count_button.value = True


    # Start at the first line, fifth column (numbering from zero).
    lcd.set_cursor_pos(0, 1)
    lcd.print("Score: "+ str(score))
 #   lcd.print(str(score))
    time.sleep(.06)



#    lcd.set_cursor_pos(0, 1)

    lcd.set_cursor_pos(1, 2)
    lcd.print("Ball Count: "+str(ball_count))
#lcd.print("#:")
    #Prints
 #   lcd.print(str(ball_count))
    time.sleep(.06)

 #   lcd.clear()
 #   lcd.print("Update!")


    # Make the screen scroll sideways
#    for i in range (0,20):
#       lcd.shift_display(1)
#       time.sleep(.25)

